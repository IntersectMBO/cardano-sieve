...
applyBlock
  ...
  datumsAndScripts = do
    ...
    execute
      ...
      (slot, headerHash)
    -- The four per-row statements are compiled once per block ('withStatement')
    -- and then re-executed per row with fresh parameter values, without
    -- recompiling the SQL. 'execute' recompiles its SQL string on every call —
    -- fine for the per-block statements above, but these four are the only ones
    -- that run per ROW, and per-row compilation of four fixed strings adds up
    -- over millions of rows. Scoping the handles to the block keeps them out
    -- of 'DbHandle'.
    unless (null created) $
    ...
    -- The stored bodies back @/datums/{hash}@ and @/scripts/{hash}@: a
    -- hash-only script UTxO can be neither interpreted nor spent without
    -- them.
    --
    -- Unlike 'created', which arrives already narrowed to the selectors,
    -- 'datumsAndScripts' is the block's full selector-blind bundle — and it
    -- cannot be narrowed to the matched outputs' datums, because an output
    -- carries only the datum HASH: the body is supplied in some
    -- transaction's witness set, often a later transaction than the output
    -- that references it, so exact filtering would mean probing a
    -- wanted-hash set for every datum and script on the sync hot path.
    -- Instead the write is gated per block: no matched output, store
    -- nothing; at least one, store the whole bundle (deduplicated by hash,
    -- and tiny in practice next to outputs).
    --
    -- The gate is "produced a matched output" and not also "or spent a
    -- tracked one" because widening it would cost a lookup per input on the
    -- sync hot path. Under a wildcard selector (how the benchmark runs) any
    -- block with transactions produces matched outputs, so the two coincide.
    -- Under a narrow selector the narrower gate stores strictly less, which
...
-- | Run a prepared row statement with these parameters: bind, executeRow, reset.
-- 'withBind' resets on the way out, so the statement is ready for the next row.
executeRow :: ToRow p => Statement -> p -> IO ()
executeRow stmt p = withBind stmt p (() <$ (nextRow stmt :: IO (Maybe (Only Int64))))
...
insertOutput conn outIns unsIns policyIndexing slot o = do
  ...
  outputNum <- outputNumOf (soOutputRef o)
  executeRow
    unsIns
  ...
  case policyIndexing of
    ...
    MaintainPolicies ->
      mapM_
        ...
        (soAssets o)
 where
  -- Must be called immediately after the INSERT OR IGNORE above with no
  -- intervening statements. 'changes' reads that INSERT's row-count: 1 means
  -- the row was new and 'lastInsertRowId' is safe to use (no SQL); 0 means
  -- the row already existed and was silently skipped, so 'lastInsertRowId' is
  -- stale — a SELECT recovers the correct output_num instead.
  outputNumOf ref = do
    ...
    if inserted > 0
      then lastInsertRowId conn
        ...
        case rows of
          Only num : _ -> pure num
          [] -> error "outputNumOf: outputs row absent immediately after INSERT OR IGNORE"

-- | Resolve a policy hash to its @policy_ids@ integer ID, inserting the row
-- on first sight.
--
-- Always two statements: @INSERT OR IGNORE@ then @SELECT@. Policies repeat
-- across many outputs, so the INSERT is ignored far more often than it
-- succeeds.
...
recordSpend spendIns delUns slot si = do
  ...
  executeRow delUns (Only (siConsumed si))
...
-- @outputs@ delete.
--
-- Undoing a spend has to return its output to @unspent@. Recording a spend
-- does two things: adds a @spends@ row and deletes the @unspent@ row. A
-- rollback deletes the @spends@ row, but that deletion of @unspent@ is not
-- automatically reversed — leaving the output in @outputs@ with no @spends@
-- row, but also absent from @unspent@. Since live UTxO queries go through
-- @unspent@, the output is permanently invisible. A single-table design with a
-- nullable @spent_at@ could undo this with one @UPDATE@; our split requires a
-- re-insert into @unspent@ from @outputs@.
--
...
rollbackAbove db@DbHandle{dbConn = conn} mSlot = do
  flushBatch db
  case mSlot of
    -- Genesis rollback: wipe every table.
    Nothing ->
      mapM_
        (execute_ conn)
    ...
    Just slot -> do
      -- @outputs@ is the permanent store; @unspent@ is the live set. When an
      -- output is spent it is removed from @unspent@. This INSERT copies it
      -- back from @outputs@, restoring outputs whose spends are being rolled
      -- back.
      --
      -- Must run before deleting from @spends@: the subquery
      -- @spends WHERE spent_slot > ?@ is used as a lookup to find which
      -- outputs to restore. Those @spends@ rows are themselves deleted
      -- afterwards (the spend is being undone), so the lookup must happen
      -- first.
      --
      -- The @created_slot <= ?@ guard excludes outputs created AND spent above
      -- the rollback point — those must disappear entirely, not be restored.
      --
      -- @spends(spent_slot)@ is indexed, so the subquery is a range scan.
      execute
        conn
        "INSERT OR IGNORE INTO unspent \
        \(output_reference, output_num, transaction_index, address, payment_credential, \
