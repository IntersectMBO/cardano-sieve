...
# kupo's baked-in RTS default is "-N -A16m -qb -qg" (all cores). Its peak RSS
# scales with -N, so a published comparison must pin it. Set e.g. KUPO_RTS=-N1
# to override; empty = kupo's own default.
KUPO_RTS="${KUPO_RTS:-}"
...
run_kupo() { # $1 = run index -> "elapsed_s max_rss_kb db_bytes cpu_time_s"
  ...
  "$KUPO_BIN" \
      ...
      --defer-db-indexes \
      --host 127.0.0.1 --port "$KUPO_PORT" \
      ${KUPO_RTS:++RTS $KUPO_RTS -RTS} >/dev/null 2>&1 &
  local pid=$!
...
log "sieve = $SIEVE_BIN"
log "kupo  = $KUPO_BIN (RTS: ${KUPO_RTS:-kupo default -N -A16m -qb -qg})"
